...
class ManimBreakpointInserter:
    ...
    def _load_breakpoint_prompt(self) -> str:
        ...
        prompt_file = Path("prompt_templates/BreakPoint.txt")
...
{script_content}"""
    ...
    def clean_code_content(self, raw_code: str) -> str:
            """
            清理代码内容，移除 markdown 标记，保留所有实质性代码行
            """
            lines = raw_code.split('\n')
            cleaned_lines = []
            
            # 1. 预先扫描：判断全篇是否包含 markdown 代码块标记
            # 如果包含，我们只提取 ``` 内部的内容
            # 如果不包含，说明整段都是代码（因为外层已经用 <<<CODE_START 提取过了），全部保留
            ...
            for line in lines:
                ...
                # 3. 核心过滤逻辑
                if has_markdown:
                    # 只有在“有Markdown模式”下，才丢弃 block 外部的内容
                    if not in_code_block:
                        continue
                
                # 4. 其他情况（在block内，或者根本没有markdown），无条件保留
                # 这样 plt.show() 和 x = ... 这种顶格代码就能活下来了

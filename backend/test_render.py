...
class ManimBatchRenderer:
    ...
    def __init__(self, input_dir: str, output_dir: str, quality: str = "h", media_dir: Optional[str] = None):
        """
        初始化渲染器
    ...
    def render_scene(self, python_file: Path, scene_class: str) -> Optional[Path]:
        ...
        try:
            ...
            # 如果用户指定了 media_dir，则添加 --media_dir 参数
            if self.media_dir:
                cmd += ["--media_dir", str(self.media_dir)]
    ...
    def find_generated_video(self, python_file: Path, scene_class: str) -> Optional[Path]:
        ...
        media_dir = self.media_dir or (python_file.parent / "media")
        if media_dir.exists():
...
示例用法:
  ...
  python batch_render_manim.py Code/KNN_9757_sections/ Video/KNN_9757_sections/ --quality m
    ...
    parser.add_argument(
        "--media_dir",
        default=None,
        help="（可选）指定 Manim 渲染的 media 输出目录。如果不提供则使用默认目录"
    )

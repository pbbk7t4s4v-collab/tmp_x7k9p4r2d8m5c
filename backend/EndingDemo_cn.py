...
class EndingScene_cn(Scene):
    def __init__(self, class_title_text="回归", avatar_image="/home/TeachMasterAppV3/backend/TeachingMaster.png", 
                 professor_name="Timo", background_image="SAI.png", 
                 school = " ", university = " ", left_logo="/home/TeachMasterAppV3/backend/logo.png", **kwargs):
    ...
    def construct(self):
        ...
        logo = ImageMobject("/home/TeachMasterAppV3/backend/TeachingMaster.png").scale(0.05).to_corner(UP + RIGHT, buff=0.5)
        ...
        if self.professor_name.strip() == "Timo":
            presenter = Text(f"由Timo创建", font="AR PL UKai CN", font_size=23, color=GREY_A)
        else:
            presenter = Text(f"由{self.professor_name}和Timo共建", font="AR PL UKai CN", font_size=23, color=GREY_A)
        website = MarkupText(
        ...
        # 3. 创建左侧内容 - 感谢词
        thank_you = Text("感谢聆听！", font="AR PL UKai CN", font_size=60, weight=BOLD)
        course_name = Text(self.class_title_text, font="AR PL UKai CN", font_size=40, slant=ITALIC)
        school = Text(self.school, font="AR PL UKai CN", font_size=32, color=GREY_B)
        university = Text(self.university, font="AR PL UKai CN", font_size=32, color=GREY_B)
        left_content = VGroup(thank_you, course_name, school, university)
        ...
        # 5. 编排动画 (与封面保持一致)
        self.play(FadeIn(logo, left_logo))
        self.wait(0.5)

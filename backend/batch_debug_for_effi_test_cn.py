...
import time
import datetime
import auto_debug_manim_cn as adm
...
def main(folder: str, render_dir: Optional[str], workers: int) -> int:
    ...
    # 统一使用 folder/media 作为渲染输出目录
    effective_render_dir = target_dir / "media"
    effective_render_dir.mkdir(parents=True, exist_ok=True)
    if not files:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as executor:
        future_to_file = {executor.submit(_run_single_debug, file_path, str(effective_render_dir)): file_path for file_path in files}
        for future in concurrent.futures.as_completed(future_to_file):# 上面改了一下渲染路径，现在英文还没改
            if future.result():
                success += 1
